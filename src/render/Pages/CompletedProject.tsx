...
const CompletedProject = () => {
  const { completedProjects, completedTasks, loading, error } = useFetchComplete();
  ...
  if (!completedTasks || completedTasks.length === 0) return <div className="text-center text-gray-500">No Projects completed</div>;

  return (
    <div className="relative w-full min-h-[85vh] border border-slate-200 rounded-lg bg-white shadow-sm p-4">
      <h2 className="text-2xl font-semibold text-primary mb-6">Completed Projects</h2>
      {completedProjects.map((project, index) => (
        ...
        >
          <div className="flex items-center gap-3">
            <i className="bi bi-folder text-gray-600 text-xl"></i>
